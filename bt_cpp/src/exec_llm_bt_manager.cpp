...
*/ 
  ...
  // path init
  std::string xml_path = ros::package::getPath("bt_cpp") + "/llm_bt_xml/";
  ...
  while (is_mission_on && ros::ok()) {
    ...
    for (const auto& entry : std::filesystem::directory_iterator(xml_path)) {
        if (entry.is_regular_file() && entry.path().extension() == ".xml") {
            ...
            std::cout << "BT file found: " << bt_filename << std::endl;
            is_bt_found = true;
    ...
    if (is_bt_found) {
      ...
      std::cout << "Renamed to: " << executed_bt_path.filename() << std::endl;
        
    } else {
      std::cout << "No BT file found. Checking again in 1 second..." << std::endl;
    }
