...
void PathRequest::construction() {
  ...
  ROS_INFO("initializing action_client_. action_client_name: %s",
           action_client_name.c_str());
  action_client_ =
  ...
  } else if (getInput<Pose3D>("goal", _goal) &&
             !getInput<bool>("is_to_object")) {
    std::cout << "going to goal" << _goal.x << ", " << _goal.y << ", "
              << _goal.z << std::endl;
  } else {
    std::cout << "going to the detected object" << std::endl;
    auto is_to_object = getInput<bool>("is_to_object");
    _is_to_object = is_to_object.value();
    object_pose_sub_ = nh_.subscribe("/object_pose", 10,
                                     &PathRequest::objectPoseCallback, this);
  }
...
void PathRequest::objectPoseCallback(
  ...
  if (_is_to_object) {
    ...
    printIfFromLastPrintHavePassedSomeSeconds(
        log.str(), duration, prev_printed_msg_, last_print_time_);
...
BT::NodeStatus PathRequest::onStart() {
  ...
  _service_client =
      nh_.serviceClient<iauv_motion_planner::GetPath>(service_name);
...
BT::NodeStatus PathRequest::onRunning() {
  ...
  if (_service_call_future.wait_for(std::chrono::seconds(1)) ==
    ...
    ROS_INFO_STREAM("Start position: " << _request.start.position.x << ", "
                                       << _request.start.position.y << ", "
                                       << _request.start.position.z);
    ROS_INFO_STREAM("Goal position: " << _request.goal.position.x << ", "
                                      << _request.goal.position.y << ", "
                                      << _request.goal.position.z);
    ...
    if (call_success) {
      ROS_INFO_STREAM(
          "[ PathRequest: FINISHED ] - call_success:" << call_success);
...
BT::NodeStatus PathRequest::tick() {
  ...
  if (prev_status == BT::NodeStatus::RUNNING) {
    ...
    if (new_status == BT::NodeStatus::IDLE) {
      throw BT::LogicError("PathRequest::onRunning() must not return IDLE");
    }
    return new_status;
  }
  return prev_status;
