...
class ROS1TopicExplorer:
    ...
    def find_battery_topic(self):
        ...
        for topic, types in topics:
            if "robotA/battery_level" in topic.lower():
...
def main():
    ...
    if((len(graph)>0) and (len(taxonomy) > 0)):
          mission_file_path = roslib.packages.get_pkg_dir('pcl_geometric_primitives_detector')+'/ace/mission_Alpha.txt'  # Path to the text file
